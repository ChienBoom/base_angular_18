...
export class MenuService {
    ...
    getData(): Observable<Navigation> {
        const compactNav: FuseNavigationItem[] = this.menuData.map(({ id, title, type, icon }) => ({
            id,
            title,
            type: 'aside',
            tooltip: title,
        ...
        const defaultNav: FuseNavigationItem[] = this.menuData;
        const futuristicNav: FuseNavigationItem[] = this.menuData.map(
            ({ id, title, type }) => ({
                id,
                title,
                type: 'group',
        ...
        const horizontalNav: FuseNavigationItem[] = this.menuData.map(
            ({ id, title, type, icon }) => ({
                id,
                title,
                type: 'group',
        ...
        const navigations: Navigation = {
            compact: cloneDeep(compactNav),
            default: cloneDeep(defaultNav),
            futuristic: cloneDeep(futuristicNav),
